--[[----------------------------------------------------
  MULTIPLAYER
  Display the 'WASTED' label with additional information
  about your killer
]]------------------------------------------------------
...
if SERVER then
  ...
  hook.Add("PlayerDeath", "gtavds_death_data", function(victim, inflictor, attacker)
    ...
    if (inflictor == attacker and attacker:IsPlayer() and IsValid(attacker:GetActiveWeapon()) and attacker:GetActiveWeapon():GetPrimaryAmmoType() > 0) then
...
if CLIENT then
  ...
  local DEFAULT = "death.weapon.generic";
  local AMMO_TYPES = {
    ["pistol"] = "death.weapon.pistol",
    ["357"] = "death.weapon.pistol",
    ["smg1"] = "death.weapon.smg",
    ["ar2"] = "death.weapon.rifle",
    ["buckshot"] = "death.weapon.shotgun",
    ["xbowbolt"] = "death.weapon.bolt",
    ["rpg_round"] = "death.weapon.explosion",
    ["rpg_missile"] = "death.weapon.explosion",
    ["grenade"] = "death.weapon.explosion",
    ["npc_grenade_frag"] = "death.weapon.explosion"
  };
  local EXPLOSION = {
    ["env_explosion"] = true,
    ["entityflame"] = true
  ...
  surface.CreateFont( "gtavds_mp_player", {
    font = "ChaletComprime-CologneSixty",
    size = ScreenScale(12),
    weight = 0,
    antialias = true,
    additive = true
  ...
  local function GetDeathString()
    if (wasSuicide) then return "death.suicide"; end
    if (wasUser) then
      if (AMMO_TYPES[weapon]) then
        return AMMO_TYPES[weapon];
      elseif (VEHICLES[weapon]) then
        return "death.vehicle";
      else
        if (wasPlayer) then
          return DEFAULT;
        else
          return "death.npc.generic";
        end
      end
    else
  ...
  local function DrawPlayerLabel(x, y, name, label)
    ...
    local size = nameSize + labelSize + ScreenScale(2);
  ...
  function GTAVDS:DrawPoly()
    local poly = {
      {x = 0, y = ScrH() * 0.433, u = 0, v = 0},
      {x = ScrW(), y = ScrH() * 0.407, u = 1, v = 0},
      {x = ScrW(), y = ScrH() * 0.645, u = 1, v = 1},
      {x = 0, y = ScrH() * 0.605, u = 0, v = 1}
    };
  ...
  function GTAVDS:DrawWastedMP(standby)
    ...
    draw.SimpleText(GTAVDS:GetWastedLabel(), "gtavds_mp_title", ScrW() * 0.5, ScrH() * y, Color(200 + 55 * anim, 40 + 215 * anim, 40 + 215 * anim), 1, 1);

    if (GetDeathString() ~= nil) then
      -- Draw label
      if (wasPlayer and not wasSuicide) then
        DrawPlayerLabel(ScrW() * 0.5, ScrH() * 0.575, attacker, GTAVDS:GetString(GetDeathString()));
      else
        local label = GTAVDS:GetString(GetDeathString());
        if (wasUser and not wasPlayer) then label = GTAVDS:GetString("death.npc.one") .. " " .. language.GetPhrase(attacker) .. " " .. label; end
